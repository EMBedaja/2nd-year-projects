...
?>
<div class="header">   
   <h3>System User Profile</h3><hr>
</div>
<div class="btn-box" style="margin-top:5px;margin-right:100px;">
<?php
 if($user != 'Staff'){
?>
...
<form method="POST" action="processes/process.user.php?action=update">
        <div id="form-block-half">
            ...
            <input <?php if($user == 'Staff'){ echo 'disabled';};?> type="text" id="fname" class="input" name="firstname" value="<?php echo $user->get_user_firstname($id);?>" placeholder="Your name..">

            <label for="lname">Last Name</label>
            <input <?php if($user == 'Staff'){ echo 'disabled';};?> type="text" id="lname" class="input" name="lastname" value="<?php echo $user->get_user_lastname($id);?>" placeholder="Your last name..">

            <label for="access">Access Level</label>
            <select <?php if($user == 'Staff'){ echo 'disabled';};?> id="access" name="access">
        ...
        <div id="form-block-half">
            ...
            <select <?php if($user == 'Staff'){ echo 'disabled';};?> id="status" name="status" disabled>
            ...
            <input <?php if($user == 'Staff'){ echo 'disabled';};?> type="email" id="email" class="input" name="email" disabled value="<?php echo $user->get_user_email($id);?>" placeholder="Your email..">
    ...
    <div class="container">
      ...
      <input type="hidden" id="userid" name="userid" value="<?php echo $id;?>"/>
        <label for="crpassword">Current Password</label>
            ...
            <input type="password" id="npassword" class="input" name="npassword" placeholder="New Password" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}"
  title="Must contain at least one  number and one uppercase and lowercase letter, and at least 8 or more characters" required>  
            <label for="copassword">Confirm Password</label>
            <input type="password" id="copassword" class="input" name="copassword" placeholder="Confirm Password" onkeyup='check();' placeholder="Confirm password.." required> 
